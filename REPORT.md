...
**First:** this pull request merges to `main`. Until then there is no commit on `main` to
pin, and a consumer pinning a branch commit would pin something that never merges.

**Then, all five in parallel.** Nothing here is shared between them: each change is one
...
- **Pin the commit that lands on `main`**, not the branch tip. The bridge squash-merges, so
  the SHA a consumer pins is the squash commit.
...
Branch `editorconfig-gate`, on pull request #43. Not merged.
